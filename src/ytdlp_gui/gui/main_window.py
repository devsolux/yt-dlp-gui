...
class YTDLPGUIApp:
    ...
    def create_menubar(self):
        ...
        try:
            ...
            # keep app robust if menu creation fails
            self.logger.error(f"Failed to create menu bar: {e}")

    def show_about(self):
        """Show About dialog with application info"""
        app_version = "1.0.0"
        author = "Baris Aydin <devsolux@gmail.com>"

        # Determine yt-dlp version robustly
        ytdlp_version = "not installed / unknown"
        try:
            # First try installed distribution metadata (works even if package internals differ)
            try:
                ytdlp_version = version("yt-dlp")
            except PackageNotFoundError:
                try:
                    ytdlp_version = version("yt_dlp")
                except PackageNotFoundError:
                    # Fall back to importing the package and reading common attributes
                    import yt_dlp as ytdlp_pkg
                    v = getattr(ytdlp_pkg, "__version__", None)
                    if isinstance(v, str) and v:
                        ytdlp_version = v
                    else:
                        # some versions expose a version module/object: try to read string attrs from it
                        mv = getattr(ytdlp_pkg, "version", None)
                        if isinstance(mv, str) and mv:
                            ytdlp_version = mv
                        else:
                            v2 = getattr(mv, "__version__", None) if mv is not None else None
                            if isinstance(v2, str) and v2:
                                ytdlp_version = v2
                            else:
                                # package is present but version string couldn't be read reliably
                                ytdlp_version = "installed (unknown version)"
        except Exception as e:
            # keep it quiet for the user but log if you have a logger
            try:
                self.logger.debug(f"Could not determine yt-dlp version: {e}")
            except Exception:
                pass
        ...
        info_text = (
            ...
            f"This GUI is a front-end for yt-dlp. See project README for license and dependency details."
        )

        try:
            about_win = ctk.CTkToplevel(self.root)
            about_win.title("About YT-DLP GUI")
            about_win.transient(self.root)
            about_win.resizable(False, False)
            about_win.grab_set()

            about_win.geometry("460x260")
            container = ctk.CTkFrame(about_win, fg_color="transparent")
            ...
            close_btn = ctk.CTkButton(container, text="Close", width=100, command=about_win.destroy)
            close_btn.pack(pady=(0, 6))
        except Exception:
            # Fallback to standard messagebox if CTkToplevel fails for any reason
            messagebox.showinfo("About YT-DLP GUI", info_text, parent=self.root)
