...
class DownloadManager:
    ...
    def _prepare_ydl_options(self, download_item: DownloadItem) -> Dict:
        ...
        if not download_item.format_info.get('audio_only'):
            ...
            if is_low_quality_selector:
                ...
                if 'vcodec^=avc1' in format_id:
                    ...
                    print(f"   📱 {height}p → Selector with H.264 priority (avoid gray filter)")
                else:
                    ...
                    print(f"   📱 {height}p → Replaced with an H.264-priority selector")
            elif 'bestvideo' in format_id:
                if height_match:
                    height = int(height_match.group(1))

                    if height <= 1080:
                        # Medium qualities: strict H.264 + FORCED AUDIO
                        # CRITICALLY IMPORTANT: Force language in selector
                        format_id = f'bestvideo[height<={height}][vcodec^=avc1]+bestaudio[language=tr]/bestvideo[height<={height}]+bestaudio[language=orig]/bestvideo[height<={height}]+bestaudio/best'
                        self.logger.info(f"Quality {height}p: H.264 codec + forced audio")
                        print(f"   {height}p -> H.264 codec + FORCED AUDIO")
                    else:
                        # High qualities: any codec + FORCED AUDIO
                        format_id = f'bestvideo[height<={height}]+bestaudio[language=tr]/bestvideo[height<={height}]+bestaudio[language=orig]/bestvideo[height<={height}]+bestaudio/best'
                        self.logger.info(f"Quality {height}p: Any codec + forced audio")
                        print(f"   🖥️ {height}p → Any codec + FORCED AUDIO")
                else:
        ...
        ydl_opts = {
            ...
            'prefer_original_language': True,  # Prefer original language
            # FORCED HTTP HEADERS FOR CONTENT
            'http_headers': {
                'Accept-Language': 'tr-TR,tr;q=0.9',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
            },
        ...
        if download_item.format_info.get('audio_only'):
            ...
            if height > 1080:
                ...
                print(f"   🔄 {height}p → Minimal FFmpeg to fix the gray filter")

            self.logger.info(f"Quality {height}p: MP4 format (compatible, no freezing)")
            print(f"   📱 {height}p → MP4 (compatible, no hangs)")
        ...
        if cookie_opts:
            ...
            print(f"   🍪 Using browser cookies to obtain the original audio")
        else:
            self.logger.warning("No cookies available - may get dubbed audio instead of original")
            print(f"   ⚠️  Cookies unavailable - you may get dubbed audio instead of the original")

        self.logger.info(
            f"Final yt-dlp options: format='{ydl_opts['format']}', cookies={'yes' if cookie_opts else 'no'}")
        print(f"   🔧 Final settings: format='{ydl_opts['format']}', cookies={'yes' if cookie_opts else 'no'}")
    ...
    @staticmethod
    def _clean_display_string(text: str) -> str:
    ...
    def _postprocessor_hook(self, d: Dict, download_id: str):
        ...
        try:
            ...
            self.logger.info(
                f"Postprocessor hook: status={d.get('status')}, postprocessor={d.get('postprocessor')}, data={d}")
